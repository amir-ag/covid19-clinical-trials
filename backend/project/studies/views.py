...
class GetAllStudies(APIView):
    def get(self, request, **kwargs):
        ...
        for i in range(studies_length):
            for j in range(i + 1, studies_length):
                if studies[i]['Latitude'] == studies[j]['Latitude'] and studies[i]['Longitude'] == studies[j]['Longitude']:
                        ...
                        if studies[i]['Latitude'] == s['Latitude'] and studies[i]['Longitude'] == s['Longitude']:
                            ...
                            for clinic in data[idx]['clinics']:
                                if studies[i]['id'] == clinic['id']:
                                   ckeck_id = 1;
                            if not ckeck_id: 
                                studies[j]['visitStudy'] = f"https://clinicaltrials.gov/ct2/show/{studies[j]['NCTId']}"
                                data[idx]['clinics'].append(studies[j])
        
                elif i < 1:
                    studies[j]['visitStudy'] = f"https://clinicaltrials.gov/ct2/show/{studies[j]['NCTId']}"
                    data.append({
                                'Latitude': studies[j]['Latitude'],
                                'Longitude': studies[j]['Longitude'],
                                'clinics': [studies[j]]
                            })
